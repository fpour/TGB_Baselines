...
def get_link_prediction_tgb_data(dataset_name: str,
                                 train_time_gran: str = 'ct',
                                 eval_time_gran: str = 'ct', time_scale: str = None):
    """
    ...
    if time_scale is not None:
        if eval_time_gran == 'dt':
            ...
            print(
                f"\tDEBUG: Val.: {dataset.root}/{dataset_name}_val_ns_" + time_scale + ".pkl")
            print(
                f"\tDEBUG: Test.: {dataset.root}/{dataset_name}_test_ns_" + time_scale + ".pkl")
            dataset.negative_sampler.load_eval_set(
                fname=f"{dataset.root}/{dataset_name}_val_ns_" + time_scale + ".pkl", split_mode="val")
            dataset.negative_sampler.load_eval_set(
                fname=f"{dataset.root}/{dataset_name}_test_ns_" + time_scale + ".pkl", split_mode="test")
        else:
            dataset.load_val_ns()
    ...
    if time_scale is not None:
        print(
            f"DEBUG: Load DTDG timestamps for all edges; Timestep: {time_scale}")
    ...
    full_data = Data(src_node_ids=np.concatenate((train_data.src_node_ids, val_data.src_node_ids, test_data.src_node_ids), axis=0),
                     dst_node_ids=np.concatenate(
                         (train_data.dst_node_ids, val_data.dst_node_ids, test_data.dst_node_ids), axis=0),
                     node_interact_times=np.concatenate(
                         (train_data.node_interact_times, val_data.node_interact_times, test_data.node_interact_times), axis=0),
                     edge_ids=np.concatenate(
                         (train_data.edge_ids, val_data.edge_ids, test_data.edge_ids), axis=0),
                     labels=np.concatenate((train_data.labels, val_data.labels, test_data.labels), axis=0))

    print("INFO: The dataset has {} interactions, involving {} different nodes".format(
...
def remove_duplicate_edges(data_split: dict):
    ...
    print(
        f"\tINFO: original number of edges: {query.shape[1]}, number of duplicated edges: {uniq.shape[1]}")
...
def get_link_pred_data_TRANS_TGB(dataset_name: str):
    ...
    edge_ids = np.array(
        [i for i in range(1, len(data.src)+1)]).astype(np.longlong)
    labels = data.y.numpy()
...
def get_node_classification_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    val_time, test_time = list(np.quantile(
        graph_df.ts, [(1 - val_ratio - test_ratio), (1 - test_ratio)]))
    ...
    val_mask = np.logical_and(node_interact_times <=
                              test_time, node_interact_times > val_time)
