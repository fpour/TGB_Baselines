...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            if self.sample_neighbor_strategy == 'time_interval_aware':
                self.nodes_neighbor_sampled_probabilities.append(self.compute_sampled_probabilities(
                    np.array([x[2] for x in sorted_per_node_neighbors])))
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        nodes_neighbor_times = np.zeros(
            (len(node_ids), num_neighbors)).astype(np.float32)
            ...
            if len(node_neighbor_ids) > 0:
                if self.sample_neighbor_strategy in ['uniform', 'time_interval_aware']:
                    ...
                    if node_neighbor_sampled_probabilities is not None:
                        ...
                        node_neighbor_sampled_probabilities = torch.softmax(torch.from_numpy(
                            node_neighbor_sampled_probabilities).float(), dim=0).numpy()
                    if self.seed is None:
                        sampled_indices = np.random.choice(a=len(
                            node_neighbor_ids), size=num_neighbors, p=node_neighbor_sampled_probabilities)
                    else:
                        sampled_indices = self.random_state.choice(a=len(
                            node_neighbor_ids), size=num_neighbors, p=node_neighbor_sampled_probabilities)
...
def get_neighbor_sampler_pyg_TD(temporal_data, start_snapshot_index: int, end_snapshot_index: int, snapshot_indices: list,
                                sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0,
                                seed: int = None):
    """
    get neighbor sampler considering that the input is a PyG TemporalData object
    :param temporal_data: PyG TemporalData
    :param start_snapshot_index (int): index of the start snapshot to extract data
    :param end_snapshot_index (int): index of the end snapshot to extract data
    :param snapshot_indices (list): a list of all snapshot indices
    :param sample_neighbor_strategy: str, how to sample historical neighbors, 'uniform', 'recent', or 'time_interval_aware''
    :param time_scaling_factor: float, a hyper-parameter that controls the sampling preference with time interval,
    a large time_scaling_factor tends to sample more on recent links, this parameter works when sample_neighbor_strategy == 'time_interval_aware'
    :param seed: int, random seed
    ...
    idx_start = snapshot_indices[start_snapshot_index][0]
    idx_end = snapshot_indices[end_snapshot_index][1] + 1

    src_node_ids = temporal_data.sources[idx_start:idx_end]
    dst_node_ids = temporal_data.destinations[idx_start:idx_end]
    edge_ids = temporal_data.edge_ids[idx_start:idx_end]
    node_interact_times = temporal_data.timestamps[idx_start:idx_end]

    # generate temporal neighorhood
    max_node_id = max(src_node_ids.max(), dst_node_ids.max())
    # the adjacency vector stores edges for each node (source or destination), undirected
    # adj_list, list of list, where each element is a list of triple tuple (node_id, edge_id, timestamp)
    # the list at the first position in adj_list is empty
    adj_list = [[] for _ in range(max_node_id + 1)]
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(src_node_ids, dst_node_ids,
                                                                     edge_ids, node_interact_times):
        adj_list[src_node_id].append(
            (dst_node_id, edge_id, node_interact_time))
        adj_list[dst_node_id].append(
            (src_node_id, edge_id, node_interact_time))
...
class NegativeEdgeSampler_local(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None,
        ...
        if self.negative_sample_strategy != 'random':
            ...
            self.possible_edges = set((src_node_id, dst_node_id)
                                      for src_node_id in self.unique_src_node_ids for dst_node_id in self.unique_dst_node_ids)
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        """
        if self.negative_sample_strategy == 'random':
            negative_src_node_ids, negative_dst_node_ids = self.random_sample(
                size=size)
    ...
    def random_sample_with_collision_check(self, size: int, batch_src_node_ids: np.ndarray, batch_dst_node_ids: np.ndarray):
        ...
        batch_edges = set((batch_src_node_id, batch_dst_node_id) for batch_src_node_id,
                          batch_dst_node_id in zip(batch_src_node_ids, batch_dst_node_ids))
    ...
    def reset_random_state(self, seed: int):
        """
        reset the random state by self.seed
        :return:
        """
