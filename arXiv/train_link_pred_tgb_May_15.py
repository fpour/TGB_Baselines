"""
python train_link_pred_tgb.py --dataset_name wikipedia --model_name DyGFormer --num_runs 1 --num_epochs 1
python train_link_pred_tgb.py --dataset_name wikipedia --model_name TGN --num_runs 1 --num_epochs 1
"""
...
from utils.DataLoader import get_idx_data_loader, get_link_prediction_data, get_link_prediction_data_TGB
from utils.EarlyStopping import EarlyStopping
from utils.load_configs import get_link_prediction_args

from tgb.linkproppred.negative_edge_loader import NegativeEdgeLoader
from tgb.linkproppred.evaluate import Evaluator
from evaluation.eval_test_linkpred_tgb import eval_link_pred_TGB


def main():

    # set internal parameters
    periodic_test_true = False
    ...
    node_raw_features, edge_raw_features, full_data, train_data, val_data, test_data, new_node_val_data, new_node_test_data = \
        get_link_prediction_data_TGB(dataset_name=args.dataset_name, val_ratio=args.val_ratio, test_ratio=args.test_ratio)
    ...
    # for the evaluation of test set with TGB 
    evaluator = Evaluator(name=args.dataset_name)
    neg_sample_strategy = 'rnd'
    neg_sample_loader = NegativeEdgeLoader(dataset_name=args.dataset_name, strategy=neg_sample_strategy, device=args.device)
    path = osp.join(osp.dirname(osp.realpath(__file__)), 'processed_data', args.dataset_name)
    neg_sample_loader.load_eval_set(split_mode='test', partial_path=path)
    ...
    for run in range(args.num_runs):
        ...
        args.seed = run
        args.save_model_name = f'{args.model_name}_seed_{args.seed}'
        ...
        for epoch in range(args.num_epochs):
            ...
            logger.info(f'Epoch: {epoch + 1}, learning rate: {optimizer.param_groups[0]["lr"]}, train loss: {np.mean(train_losses):.4f}, elapsed time (s): {time.time() - start_epoch:.4f}')
        ...
        logger.info(f'get final performance on dataset {args.dataset_name}; evaluate the best model on the validation set...')
        ...
        test_metrics = eval_link_pred_TGB(model_name=args.model_name, model=model, neighbor_sampler=full_neighbor_sampler, 
                                        ...
                                        split_mode='test', negative_edge_loader=neg_sample_loader, 
                                        evaluator=evaluator, k_value=10)
        logger.info(f'test elapsed time (s): {time.time() - start_test:.4f}')
        ...
        for metric_name, metric_value in test_metrics.items():
            ...
            test_metric_dict[metric_name] = metric_value
        ...
        logger.info(f"run {run} total elapsed time (s): {time.time() - start_run:.4f}")
